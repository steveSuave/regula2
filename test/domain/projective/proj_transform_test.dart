...
void main() {
  ...
  group('Euclidean constructors (affine agreement)', () {
    Glados2(any.vec2, any.vec2).test('translation moves by the delta',
        (p, t) {
    ...
    Glados3(any.vec2, any.vec2, any.vec2)
        .test('reflection agrees with LineEq.reflect', (v, p, q) {
      if (p.closeTo(q, 1e-3)) {
        return;
      }
    ...
    Glados2(any.vec2, any.vec2)
        .test('point reflection: the center is the midpoint', (p, c) {
    ...
    Glados3(any.vec2, any.vec2, any.component)
        .test('homothety agrees with the affine formula', (p, c, r) {
    ...
    Glados(any.vec2).test('homothety fixes its center; ratio 1 is identity',
      ...
      expect(ProjTransform.homothety(center, 7.5)
          .apply(center)
          .closeTo(center), isTrue);
      expect(ProjTransform.homothety(center, 1)
          .closeTo(ProjTransform.identity), isTrue);
    });
  });

  group('composition and adjugate', () {
    Glados3(any.similarity, any.similarity, any.projPoint)
        .test('compose applies the right factor first', (m, n, p) {
    ...
    Glados(any.similarity).test(
        'a similarity composed with its adjugate is the identity', (m) {
    ...
    Glados2(any.similarity, any.projPoint)
        .test('adjugate round-trips points', (m, p) {
      expect(m.adjugate.apply(m.apply(p)).closeTo(p), isTrue);
    });

    Glados2(any.projTransform, any.projPoint).test(
        'general invertible transforms round-trip points', (m, p) {
      final f2 = m.norm2;
  ...
  group('covariance (points, lines, conics transform together)', () {
    Glados3(any.projTransform, any.projPoint, any.projPoint)
        .test('the join of images is the image of the join', (m, p, q) {
      final line = p.join(q);
    ...
    Glados2(any.similarity, any.vec2)
        .test('intersection points of a line and a circle map onto the '
            'image conic and image line', (m, c) {
    ...
    Glados2(any.similarity, any.projPoint)
        .test('evaluate scales by det² under the congruence', (m, p) {
    ...
    test('rigid motions carry a circle to the expected circle', () {
      ...
      final m = ProjTransform.translation(3, -4).compose(
          ProjTransform.rotation(ProjPoint.real(0.5, 0.25), 1.2));
      final expectedCenter = const Vec2(0.5, 0.25) +
  ...
  group('rescaling invariance', () {
    Glados3(any.projTransform, any.projPoint, any.nonZeroComplex)
        .test('a rescaled matrix is the same map', (m, p, k) {
    ...
    Glados2(any.similarity, any.nonZeroComplex)
        .test('line and conic images are rescaling-invariant', (m, k) {
      final scaled = m.scaledBy(k);
    ...
    Glados2(any.similarity, any.nonZeroComplex)
        .test('isReal is invariant under complex rescaling', (m, k) {
  ...
  group('degenerate inputs propagate', () {
    ...
    test('a homothety of ratio 0 maps everything to its center', () {
      ...
      expect(m.apply(ProjPoint.real(5, 7)).closeTo(ProjPoint.real(2, -1)),
          isTrue);
      expect(m.det.abs, lessThanOrEqualTo(1e-15));
    });
  });
